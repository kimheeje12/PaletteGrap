...
public class Activity_MasterpieceUpload extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_masterpiece_upload);
            ...
            public void onClick(View view) {
                ...
                if (masterpiece_image.getDrawable() == null) {
                    ...
                    Retrofit retrofit = new Retrofit.Builder()
                            .baseUrl(MasterpieceUpload.MasterpieceUpload_URL)
                            .addConverterFactory(ScalarsConverterFactory.create()) //HTTP 통신 시에 주고받는 데이터 형태를 변환시켜 주는 컨버터 지정 설정
                            .build();
                    MasterpieceUpload api = retrofit.create(MasterpieceUpload.class);

                    File file = new File(photoroute); //절대경로를 가져와서 file 객체를 만들었다=> 이미지 파일을 만듦

                    RequestBody requestBody = RequestBody.create(MediaType.parse("text/plain"), loginemail); //이메일
                    RequestBody requestBody2 = RequestBody.create(MediaType.parse("text/plain"), masterpiece_story.getText().toString()); //명화 내용
                    RequestBody requestBody3 = RequestBody.create(MediaType.parse("text/plain"), masterpiece_title.getText().toString()); //명화 제목
                    RequestBody requestBody4 = RequestBody.create(MediaType.parse("text/plain"), masterpiece_artist.getText().toString()); //명화 작가
                    ...
                    Call<String> call = api.MasterpieceUpload(requestBody, requestBody2, requestBody3, requestBody4, image);
                    call.enqueue(new Callback<String>() //enqueue: 데이터를 입력하는 함수
                    {
                        ...
                        public void onResponse(@NonNull Call<String> call, @NonNull Response<String> response) {
                            if (response.isSuccessful() && response.body() != null) {
                                ...
                                Intent intent = new Intent(Activity_MasterpieceUpload.this, Activity_Masterpiece.class);
            ...
            new ActivityResultCallback<ActivityResult>() {
                ...
                public void onActivityResult(ActivityResult result) {
                    if (result.getResultCode() == RESULT_OK) {
                        ...
                        photoroute = createCopyAndReturnRealPath(Activity_MasterpieceUpload.this, uri); // 절대경로 가져오기!

                        Glide.with(Activity_MasterpieceUpload.this).load(uri).into(masterpiece_image);
